...
def start(message):
    ...
    if not check_fine(message):
        ...
        bot.send_message(message.chat.id, f"Здравствуйте, {message.from_user.first_name}, желаете ли вы начать помогать городу?",
                         reply_markup=markup)
...
def main_menu(message):
    ...
    bot.send_message(message.chat.id, "Это главное меню. Отсюда вы можете отправить заявку или посмотреть статистику.", reply_markup=markup)
...
def check_geo(message):
    if not check_fine(message):
        ...
        back = types.KeyboardButton(text="Отмена❌")
        markup.add(back)
        bot.send_message(message.chat.id, f"Геолокация принята! Ваша заявка отправлена!")
        write_coord(message.location.longitude, message.location.latitude)
        count_trash = cur.execute("""SELECT count_trash FROM Users WHERE tg_id=(?)""", (message.from_user.id,)).fetchone()
